 ...
 *  - `internal.functions.devAssistant.*` → `getDevAssistantRefs().*`
 ...
 *
 * Every function is a MODULE-LEVEL const built directly with Convex's `*Generic`
 * builders — NOT returned from a factory — so a consumer's generated `api` /
 * `internal` types survive (see `../holder` for the full why). Config is read
 * LAZILY inside each handler via `getDevAssistantConfig()` / `getDevAssistantRefs()`;
 * the consumer's config module sets it before any handler runs.
 */
...
export const getThreadHistory = internalQueryGeneric({
...
export const getBug = internalQueryGeneric({
  args: { bugId: v.id("devBugs") },
  handler: async (ctx: any, args) => await ctx.db.get(args.bugId),
});

export const getBugByRoutineRunId = internalQueryGeneric({
...
export const getOriginatorAttribution = internalQueryGeneric({
  args: { bugId: v.id("devBugs") },
  handler: async (ctx: any, args) => {
...
export const listOpenPrBugs = internalQueryGeneric({
  args: {},
...
export const markDispatched = internalMutationGeneric({
...
export const markSpecDispatched = internalMutationGeneric({
  args: {
    bugId: v.id("devBugs"),
...
export const markReviewDispatched = internalMutationGeneric({
  args: { bugId: v.id("devBugs"), routineRunId: v.string() },
  handler: async (ctx: any, args) => {
    const bug = await ctx.db.get(args.bugId);
    if (!bug) return { alreadyDispatched: true };
...
export const markFixDispatched = internalMutationGeneric({
  args: { bugId: v.id("devBugs"), routineRunId: v.string() },
  handler: async (ctx: any, args) => {
    ...
    await insertThreadMessage(
      ...
      args.bugId,
      "system",
      `AI is addressing the review feedback (round ${fixRound} of ${cfg.maxFixRounds})`,
    );
    return { alreadyDispatched: false };
  },
});

export const setGithubIssue = internalMutationGeneric({
  args: {
    bugId: v.id("devBugs"),
...
export const recordDispatchError = internalMutationGeneric({
...
export const addSystemThreadMessage = internalMutationGeneric({
...
export const recordProductionDeployOutcome = internalMutationGeneric({
  args: {
    bugId: v.id("devBugs"),
    ok: v.boolean(),
...
export const recordMergeFromAppFailure = internalMutationGeneric({
...
export const applyCallback = internalMutationGeneric({
  args: {
    bugId: v.id("devBugs"),
...
export const handleGithubPrClosed = internalMutationGeneric({
  args: {
    branchRef: v.string(),
    prUrl: v.optional(v.string()),
    merged: v.boolean(),
...
export const handleWorkflowRunEvent = internalMutationGeneric({
  args: {
    action: v.string(),
    name: v.string(),
...
export const getBugForReview = queryGeneric({
...
export const rejectBug = mutationGeneric({
  args: { token: v.string(), bugId: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    await requireSuperAdmin(ctx, args.token);
    const bug = await ctx.db.get(args.bugId);
    if (!bug) throw new Error("Bug not found");
    await applyStatusTransition(ctx, bug, "REJECTED", getDevAssistantRefs());
    await ctx.db.patch(args.bugId, {
...
export const markBugMerged = mutationGeneric({
  args: { token: v.string(), bugId: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    await requireSuperAdmin(ctx, args.token);
    const bug = await ctx.db.get(args.bugId);
    if (!bug) throw new Error("Bug not found");
    await applyStatusTransition(ctx, bug, "MERGED", getDevAssistantRefs());
    return { ok: true };
  },
});

export const retryDispatch = mutationGeneric({
  args: { token: v.string(), bugId: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    await requireSuperAdmin(ctx, args.token);
    const bug = await ctx.db.get(args.bugId);
    ...
    await ctx.scheduler.runAfter(0, getDevAssistantRefs().actions.dispatchBug, {
      bugId: bug._id,
