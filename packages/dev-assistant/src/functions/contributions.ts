 ...
 *
 * Every function is a MODULE-LEVEL const built directly with the `*Generic`
 * builders (NOT a factory) so a consumer's generated `api` types survive (see
 * `../holder`). Config/refs are read LAZILY inside each handler.
 */
...
export const getGithubUsername = queryGeneric({
  args: { token: v.string() },
...
export const setGithubUsername = mutationGeneric({
...
export const approveSpec = mutationGeneric({
...
export const startBuild = mutationGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    const cfg = getDevAssistantConfig();
    const refs = getDevAssistantRefs();
...
export const unarchive = mutationGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    const user = await requireContributor(ctx, args.token);
    const bug = await ctx.db.get(args.id);
...
export const postMessage = mutationGeneric({
  args: {
    token: v.string(),
    id: v.id("devBugs"),
    body: v.string(),
...
export const confirmStaging = mutationGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    const cfg = getDevAssistantConfig();
    const user = await requireContributor(ctx, args.token);
...
export const reportStagingIssue = mutationGeneric({
...
export const mergeNow = mutationGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    const refs = getDevAssistantRefs();
    const user = await requireContributor(ctx, args.token);
...
export const promoteToProduction = mutationGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    const cfg = getDevAssistantConfig();
    const refs = getDevAssistantRefs();
...
export const getThread = queryGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
...
export const myContributions = queryGeneric({
  args: { token: v.string() },
  handler: async (ctx: any, args) => {
    const user = await requireContributor(ctx, args.token);
...
export const getContribution = queryGeneric({
  args: { token: v.string(), id: v.id("devBugs") },
  handler: async (ctx: any, args) => {
    await requireContributor(ctx, args.token);
    const bug = await ctx.db.get(args.id);
