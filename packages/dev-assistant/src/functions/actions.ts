...
export function makeActionsFunctions(
  cfg: ResolvedDevAssistantConfig,
  refs: DevAssistantRefs,
) {
  const resolveShots = (urls: string[] | undefined): string[] =>
    (urls ?? [])
  ...
  const applyGithubConfirmedMerge = async (
    ctx: any,
  ...
  const reconcileMergedPrs = internalActionGeneric({
    args: {},
  ...
  const handleRoutineCallback = internalActionGeneric({
    args: {
      bugId: v.id("devBugs"),
